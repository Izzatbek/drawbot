...
#style flags
default = _wx.SYSTEM_MENU | _wx.CAPTION | _wx.CLOSE_BOX | _wx.MINIMIZE_BOX
no_close = _wx.DEFAULT_FRAME_STYLE & (~_wx.CLOSE_BOX)
no_minimize = _wx.SYSTEM_MENU | _wx.CAPTION | _wx.CLOSE_BOX

class window():
    def __init__(self, menus=False, _make_panel=True, style=_wx.DEFAULT_FRAME_STYLE, x=0, y=0, width=600, height=600, title='VPython', exitfn=None):
        global _do_loop
        _do_loop = True # if a window has been made, execute wait loop at exit
        self.menus = menus
        if _plat == 'Macintosh': y += 20 # 20-pixel menu bar at top of screen
        if _plat == 'Unix':
            if (x <= 65): x = 0 # 65-pixel band (launcher) at left of screen for Ubuntu 12.04
            y += 25 # 25-pixel band (menus etc.) at top of screen for Ubuntu 12.04
##        if _plat == 'Unix': height -= 25 # correction for Xubuntu; not understood
        ...
        if (exitfn == None):
            ...
            self.win.Bind(_wx.EVT_CLOSE, exitfn)
...
class _mouseTracker:
    ...
    def isZooming(self, evt, userzoom=True, userspin=True):
        ...
        elif self.leftIsDown and evt.ControlDown() and (not _plat == 'Macintosh'):
            #
            # Weird... if the user holds the cmd key, evt.ControlDown() returns True even if it's a lie.
            # So... we don't know if it's *really* down or not. ;-( On the mac.. ignore ctrl-left-click
            #
            zooming = 1
...
##            apply(self._dispatch_event, (name,), kw)  # apply doesn't exist in Python 3
    ...
    def _make_canvas(self, parent):
        ...
        if _plat == 'Unix':
            y = 0
            h = h
        if not self.standard: # place canvas in window already created by user
            x = self._x
            y = self._y
            w = self._width
            h = self._height
        c = self._canvas = _wx.glcanvas.GLCanvas(parent, -1, pos=(x, y), size=(w, h))
    ...
    def _report_resize(self):
        ...
        if not self.standard: # canvas was placed in user-created window
            w = self._width
            h = self._height
...
### Maybe there's no way to detect that a loop without a rate call?
